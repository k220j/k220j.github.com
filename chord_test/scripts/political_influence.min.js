...
function initialize() { //초기화
	if (totalContributions = 0, renderLinks = [], cands = [], pacs = [], contr = [], "house" == office) { //전역변수
		var a = {},
			b = {};
...
function node_onMouseOver(a, b) {
	var contrastdata=10000;
	var arr=[a.Amount,contrastdata];
	console.log(arr);
	var k = d3.select("#orignal").transition().duration(500).attr("height","150px");
	var l = d3.select("#backup").transition().duration(500).attr("height","150px");
	
	var c = d3.event.pageX + 15;
	
	if (c + 250 > window.innerWidth && (c = d3.event.pageX - 280), "CAND" == b) {
		if (a.depth < 2) return;
		toolTip.transition().duration(200).style("opacity", ".9"), header1.text("Congress"), header.text(a.CAND_NAME), header2.text("Total Recieved: " + formatCurrency(Number(a.Amount))), toolTip.style("left", c + "px").style("top", d3.event.pageY - 150 + "px").style("height", "400px"), highlightLinks(a, !0)
	} else
		"CONTRIBUTION" == b ? (toolTip.transition()
				.duration(200)
				.style("opacity", ".9"), 
					header1.text(pacsById[office + "_" + a.CMTE_ID].CMTE_NM),
					header.text(a.CAND_NAME),
					header2.text(formatCurrency(Number(a.TRANSACTION_AMT)) + " on " + a.Month + "/" + a.Day + "/" + a.Year), 
				...
				highlightLink(a, !0)) : "PAC" == b && (toolTip.transition().duration(200).style("opacity", ".9"),
						header1.text("Political Action Committee"), header.text(pacsById[office + "_" + a.label].CMTE_NM), 
						header2.text("Total Contributions: " + formatCurrency(pacsById[office + "_" + a.label].Amount)),
						toolTip.style("left", d3.event.pageX + 15 + "px").style("top", d3.event.pageY - 75 + "px").style("height", "510px"), 
...
function node_onMouseOut(a, b) {
	"CAND" == b ? highlightLinks(a, !1) : "CONTRIBUTION" == b ? highlightLink(a, !1) : "PAC" == b && highlightLinks(chordsById[a.label], !1), toolTip.transition().duration(500).style("opacity", "0")
			console.log("나온다.");
	var k = d3.select("#backup").attr("width","20px").attr("height","0px");
	var l = d3.select("#orignal").attr("width","20px").attr("height","0px");
...
function fetchData() {
	dataCalls = [], addStream("scripts/data/Candidates_House.csv", onFetchCandidatesHouse), addStream("scripts/data/Candidates_Senate.csv", onFetchCandidatesSenate), addStream("scripts/data/Contributions_House.csv", onFetchContributionsHouse), addStream("scripts/data/Contributions_Senate.csv", onFetchContributionsSenate), addStream("scripts/data/Pacs_House.csv", onFetchPacsHouse), addStream("scripts/data/Pacs_Senate.csv", onFetchPacsSenate), startFetch()
}
...
var svg = d3.select(document.getElementById("svgDiv")).style("width", 2 * outerRadius + 200 + "px").style("height", 2 * outerRadius + 200 + "px").append("svg").attr("id", "svg").style("width", 2 * outerRadius + 200 + "px").style("height", 2 * outerRadius + 200 + "px"),
	...
	rectGraph = d3.select(document.getElementById("rectGraph1")),
